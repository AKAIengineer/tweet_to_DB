...
def insert(tweet_id, user_id, username, text, url):
    connection = connect(host, user, password, db)
    with connection.cursor() as cursor:
        # Create a new record
        sql = "INSERT INTO `tweet` (`tweet_id`, `user_id`, `username`, `text`, `url`) VALUES (%s, %s, %s, %s, %s)"
        cursor.execute(sql, (tweet_id, user_id, username, text, url))
    connection.commit()
    connection.close()
                ...
                for response_line in response.iter_lines(chunk_size=10240):
                    if response_line:
                        ...
                        try:
                            ...
                            print("txtに書き込みエラーだよ")
                        try:
                            insert(tweet_id, user_id, username, text, url)
                        except Exception:
                            print("DBに挿入エラーだよ")
